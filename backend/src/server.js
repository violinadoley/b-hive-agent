...
function readAllEvents() {
  ...
  const lines = text
    ...
    .map((x) => x.trim())
    .filter(Boolean);
  return lines.map(safeParseJsonLine).filter(Boolean);
}
...
function getRuntimeStatus() {
  ...
  const heartbeat = readJsonFileOrNull(MONITOR_HEARTBEAT_PATH);
  const monitorState = readJsonFileOrNull(MONITOR_STATE_PATH);
  return {
...
server.listen(PORT, () => {
  ...
  console.log(`[api] decision log source: ${getLogPath()}`);
});
